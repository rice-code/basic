...
class FileNamespaceTest extends TestCase
{
    ...
    public function testAnalysisWithDifferentLines(): void
    {
        ...
        // 测试use语句行（有别名）
        $testLine = 'use App\Database as DB;';
        $this->assertFalse(
            $this->fileParser->analysis($testNamespace, $testLine),
            '分析use语句行（有别名）应该返回false'
        );
        $alias = $this->fileParser->getAlias();
        $uses  = $this->fileParser->getUses();
        $this->assertArrayHasKey($testNamespace, $alias, '应包含别名信息');
        $this->assertArrayHasKey('DB', $alias[$testNamespace], '应包含DB别名');
        $this->assertEquals('Database', $alias[$testNamespace]['DB'], 'DB别名应对应Database类');
        // 验证使用了别名的原始类名是否在uses中
        $this->assertArrayHasKey('Database', $uses[$testNamespace], '应包含Database类');
        $this->assertEquals('App', $uses[$testNamespace]['Database'], 'Database应对应App命名空间');

        // 测试类定义行
        $classLine = 'class TestClass extends BaseClass implements Interface {}';
        $this->assertTrue($this->fileParser->analysis($testNamespace, $classLine), '分析类定义行应返回true');
    }
}
